...
-- [[Configuração de usar Utito:]]
local skillsemutito = 192
macro(200, "Utito", function()
    local skillteste1 = player:getSkillLevel(2)
    local skillteste2 = player:getSkillLevel(3)
    local skillteste3 = player:getSkillLevel(4)
    if skillteste1 > skillteste2 and skillteste1 > skillteste3 then
      local spell = "utito tempo"
      if not isInPz() and g_game.isAttacking() and skillteste1 > (skillsemutito - 1) then say(spell) end
    else
    if skillteste2 > skillteste1 and skillteste2 > skillteste3 then
      local spell = "utito tempo"
      if not isInPz() and g_game.isAttacking() and skillteste2 > (skillsemutito - 1) then say(spell) end
    else
      local spell = "utito tempo"
      if not isInPz() and skillteste3 > (skillsemutito - 1) then say(spell) end
    end
    end
end)
...
local multiTargetSpell = 'exori super gran'
local distance = 3
...
addTextEdit("Spell1", storage.Spell1 or "Exori Hur", function(widget, text) 
storage.Spell1 = text
end)
addTextEdit("Spell2", storage.Spell2 or "Exori Super Gran", function(widget, text) 
storage.Spell2 = text
end)

--[[Auto exeta res com textwindow para escolher feitiço para servidores personalizados]]

macro(500, "Auto RES",  function()
  say(storage.ExetaText)
end)
addTextEdit("'A Mlk, aqui é BR", storage.ExetaText or "Exeta Res", function(widget, text) 
...
macro(4000,"Super Gran - Spot 7",function()
if g_game.isAttacking() then say("exori super gran") end
end)
